...
class CompliteProfileController extends GetxController {
  ...
  Future<bool> createProfileData(String token, CreateProfileParams params) async {
    ...
    final response =
        await NetworkCaller().postRequest(Urls.createProfile, token: token, body: params.toJson());
    _inProgress = false;
    if (response.isSuccess) {
      _profile = Profile.fromJson(response.responseData['data']);
      await Get.find<AuthController>().saveUserDetailes(token, _profile);
      update();
      return true;
    } else {
